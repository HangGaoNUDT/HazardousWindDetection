...
class cluster_NN_without_linear_layer(object):
    def __init__(self, encoded_dim=4,cluster_dim=4):
        ...
        self.pilot_center_cluster_features = center_encoded
        ...
        self.calm_center_cluster_features = center_encoded  # self.clusnn_setup(center_encoded)
    ...
    def train(self, Data, pilot_semi_, train_steps, BATCH_SIZE, imbalance_beta_values=np.array([1,10,100,1000]),
        ...
        # '''# pre-train'''
        ae_ckpt_path = os.path.join('ae_ckpt',f'model_encoder_without_linear_layer.ckpt')
        ...
        '''# semi-supervise '''
        sdec_ckpt_path = os.path.join('sdec_ckpt', 'model_supervise_without_linear_layer.ckpt')
    ...
    def cluster_visualization_tsne_test(self, hazard_id, cluster_features_all_, pilot_train_cluster_features_, calm_train_cluster_features_,
        ...
        plt.xticks(())
        plt.yticks(())
        plt.savefig(f'../figures/{i_step}steps_without_linear_layer.png',dpi=600)  #
        plt.savefig(f'../figures/{i_step}steps_without_linear_layer.pdf')
