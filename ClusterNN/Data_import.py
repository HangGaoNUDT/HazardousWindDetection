...
class data_frame(object):
    def __init__(self,calm_train_num = 10, calm_range_set = 5):
        ...
        self.still_index = np.argsort(np.max(self.unlabeled_data_v_r, axis=1) - np.min(self.unlabeled_data_v_r, axis=1))
    ...
    def calm_test_generator(self,calm_ratio=1):
        self.calm_test_num = int(calm_ratio*self.pilot_test_v_r.shape[0])
        calm_test = self.unlabeled_data_v_r[self.still_index[:self.calm_test_num], ::].reshape(
            (self.calm_test_num, self.pilot_train_v_r.shape[1]))
        return calm_test
